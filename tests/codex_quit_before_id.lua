-- Run: nvim --headless --clean -l tests/codex_quit_before_id.lua
...
assert(#entries == 1, "Codex session is persisted during quit")
assert(entries[1].id == session_id, "Codex session id captured during quit")

vim.cmd.bwipeout({ args = { tostring(bufnr) }, bang = true })
print("codex_quit_before_id OK")
